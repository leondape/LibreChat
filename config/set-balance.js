...
(async () => {
  ...
  // Load balance configuration from librechat.yaml first, then fall back to environment variables
  ...
  if (!balance.enabled) {
    console.red(
      'Error: Balance system is not enabled! Please configure it in librechat.yaml:\n' +
        'balance:\n' +
        '  enabled: true\n' +
        'Or set CHECK_BALANCE=true in .env file',
    );
    silentExit(1);
  }
  ...
  let balanceRecord = await Balance.findOne({ user: user._id }).lean();
  if (!balanceRecord) {
    console.purple('User has no balance!');
  } else {
    console.purple(`Current Balance: ${balanceRecord.tokenCredits}`);
  }
